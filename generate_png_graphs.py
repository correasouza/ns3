...
def create_comparison_bar_chart(data_sem, data_com, output_file, title, ylabel, colors=None):
    ...
    # IMPORTANTE: Obter nomes dos vídeos ANTES de agregar (preferência akiyo/bowing)
    video1_name, video2_name = choose_video_order(data_sem, data_com)
    ...
    bars2 = ax.bar(positions - 0.5*bar_width, v1_com, bar_width, 
                   label=f'{video1_name.capitalize()} - COM SDN', color=v1_colors[1], edgecolor='black', linewidth=0.5)
    bars3 = ax.bar(positions + 0.5*bar_width, v2_sem, bar_width, 
                   label=f'{video2_name.capitalize()} - SEM SDN', color=v2_colors[0], edgecolor='black', linewidth=0.5)
    bars4 = ax.bar(positions + 1.5*bar_width, v2_com, bar_width, 
                   label=f'{video2_name.capitalize()} - COM SDN', color=v2_colors[1], edgecolor='black', linewidth=0.5)
    ...
    ax.legend(loc='upper left', bbox_to_anchor=(1.02, 1), borderaxespad=0., fontsize=9, ncol=2)
...
def create_video_comparison_chart(data_sem, data_com, output_file, title, ylabel):
    ...
    # Usa cores por vídeo e mantém barras SEM/COM lado a lado com cores consistentes
    # Escolhe cores para cada vídeo (sem/com)
    color_map = {
        'akiyo': ('#1f77b4', '#86bff0'),
        'bowing': ('#c0392b', '#f39c94')
    }
    sem_colors = [color_map.get(v.lower(), ('#3498db', '#e74c3c'))[0] for v in video_names]
    com_colors = [color_map.get(v.lower(), ('#3498db', '#e74c3c'))[1] for v in video_names]

    bars1 = ax.bar(x - width/2, sem_means, width, yerr=sem_std, label='SEM SDN',
                   color=sem_colors, capsize=5, edgecolor='black', linewidth=0.5)
    bars2 = ax.bar(x + width/2, com_means, width, yerr=com_std, label='COM SDN',
                   color=com_colors, capsize=5, edgecolor='black', linewidth=0.5)
