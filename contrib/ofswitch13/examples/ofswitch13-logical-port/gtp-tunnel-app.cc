...
GtpTunnelApp::GtpTunnelApp(Ptr<VirtualNetDevice> logicalPort, Ptr<CsmaNetDevice> physicalDev)
{
    NS_LOG_FUNCTION(this << logicalPort << physicalDev);

    // Save the pointers and set the send callback.
    m_logicalPort = logicalPort;
    m_logicalPort->SetSendCallback(MakeCallback(&GtpTunnelApp::RecvFromLogicalPort, this));
...
bool
GtpTunnelApp::RecvFromLogicalPort(Ptr<Packet> packet,
                                  ...
                                  uint16_t protocolNo)
{
    ...
    NS_LOG_DEBUG("Send packet " << packet->GetUid() << " to tunnel with TEID " << teid << " IP "
                                << ipv4Addr << " port " << m_port);
...
GtpTunnelApp::RecvFromTunnelSocket(Ptr<Socket> socket)
{
    ...
    NS_LOG_DEBUG("Received packet " << packet->GetUid() << " from tunnel with TEID "
                                    << gtpu.GetTeid());
...
GtpTunnelApp::StartApplication()
{
    ...
    m_tunnelSocket = Socket::CreateSocket(GetNode(), TypeId::LookupByName("ns3::UdpSocketFactory"));
    m_tunnelSocket->Bind(InetSocketAddress(ipv4Addr, m_port));
    m_tunnelSocket->BindToNetDevice(m_physicalDev);
    m_tunnelSocket->SetRecvCallback(MakeCallback(&GtpTunnelApp::RecvFromTunnelSocket, this));
}

void
GtpTunnelApp::AddHeader(Ptr<Packet> packet,
                        Mac48Address source,
                        Mac48Address dest,
                        uint16_t protocolNo)
{
