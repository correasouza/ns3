...
{
    static TypeId tid =
        TypeId("ns3::OFSwitch13StatsCalculator")
            .SetParent<Object>()
            .SetGroupName("OFSwitch13")
            .AddConstructor<OFSwitch13StatsCalculator>()
            .AddAttribute("EwmaAlpha",
                          "The EWMA alpha parameter for averaged statistics.",
                          DoubleValue(0.2),
...
OFSwitch13StatsCalculator::HookSinks(Ptr<OFSwitch13Device> device)
{
    ...
    *m_wrapper->GetStream() << boolalpha << right << fixed << setprecision(3)
                            << " " << setw(8) << "TimeSec"
                            << " " << setw(12) << "LoaKbps"
                            << " " << setw(7) << "LoaUsag"
                            << " " << setw(7) << "Packets"
    ...
    if (m_details)
    {
        for (size_t i = 0; i < m_device->GetNPipelineTables(); i++)
        {
            ...
            *m_wrapper->GetStream() << " " << setw(7) << field1
                                    << " " << setw(7) << field2;
        }
    }
    ...
    *m_wrapper->GetStream() << std::endl;
    ...
    device->TraceConnectWithoutContext(
        "DatapathTimeout",
        MakeCallback(&OFSwitch13StatsCalculator::NotifyDatapathTimeout,
                     Ptr<OFSwitch13StatsCalculator>(this)));
    device->TraceConnectWithoutContext(
        "OverloadDrop",
        MakeCallback(&OFSwitch13StatsCalculator::NotifyOverloadDrop,
                     Ptr<OFSwitch13StatsCalculator>(this)));
    device->TraceConnectWithoutContext(
        "MeterDrop",
        MakeCallback(&OFSwitch13StatsCalculator::NotifyMeterDrop,
                     Ptr<OFSwitch13StatsCalculator>(this)));
    device->TraceConnectWithoutContext(
        "TableDrop",
        MakeCallback(&OFSwitch13StatsCalculator::NotifyTableDrop,
                     Ptr<OFSwitch13StatsCalculator>(this)));
    device->TraceConnectWithoutContext(
        "PipelinePacket",
        MakeCallback(&OFSwitch13StatsCalculator::NotifyPipelinePacket,
                     Ptr<OFSwitch13StatsCalculator>(this)));
...
OFSwitch13StatsCalculator::GetAvgCpuUsage() const
{
    ...
    return std::round(static_cast<double>(GetEwmaCpuLoad().GetBitRate()) * 100 /
...
OFSwitch13StatsCalculator::GetAvgFlowTableUsage(uint8_t tableId) const
{
    ...
    return std::round(static_cast<double>(GetEwmaFlowTableEntries(tableId)) * 100 /
...
OFSwitch13StatsCalculator::NotifyConstructionCompleted()
{
    ...
    Simulator::Schedule(m_timeout, &OFSwitch13StatsCalculator::DumpStatistics, this);
...
OFSwitch13StatsCalculator::NotifyDatapathTimeout(Ptr<const OFSwitch13Device> device)
{
    ...
    m_ewmaBufferEntries =
        m_alpha * m_device->GetBufferEntries() + (1 - m_alpha) * m_ewmaBufferEntries;
    m_ewmaCpuLoad =
        m_alpha * m_device->GetCpuLoad().GetBitRate() + (1 - m_alpha) * m_ewmaCpuLoad;
    m_ewmaSumFlowEntries =
        m_alpha * m_device->GetSumFlowEntries() + (1 - m_alpha) * m_ewmaSumFlowEntries;
    m_ewmaGroupEntries =
        m_alpha * m_device->GetGroupTableEntries() + (1 - m_alpha) * m_ewmaGroupEntries;
    m_ewmaMeterEntries =
        m_alpha * m_device->GetMeterTableEntries() + (1 - m_alpha) * m_ewmaMeterEntries;
    m_ewmaPipelineDelay =
        m_alpha * m_device->GetPipelineDelay().GetDouble() + (1 - m_alpha) * m_ewmaPipelineDelay;

    for (size_t i = 0; i < m_device->GetNPipelineTables(); i++)
    {
        m_ewmaFlowEntries.at(i) =
            m_alpha * m_device->GetFlowTableEntries(i) + (1 - m_alpha) * m_ewmaFlowEntries.at(i);
    }
    // clang-format on
}
...
void
OFSwitch13StatsCalculator::NotifyMeterDrop(Ptr<const Packet> packet, uint32_t meterId)
{
...
void
OFSwitch13StatsCalculator::NotifyTableDrop(Ptr<const Packet> packet, uint8_t tableId)
{
...
OFSwitch13StatsCalculator::DumpStatistics()
{
    ...
    if (cpuCapy)
    {
        cpuUsage = std::round(static_cast<double>(cpuLoad) * 100 / static_cast<double>(cpuCapy));
    }

    // Print statistics to file.
    // clang-format off
    *m_wrapper->GetStream() << " " << setw(8) << Simulator::Now().GetSeconds()
    ...
    if (m_details)
    {
        for (size_t i = 0; i < m_device->GetNPipelineTables(); i++)
        {
            *m_wrapper->GetStream() << " " << setw(7) << GetEwmaFlowTableEntries(i)
