...
OFSwitch13Controller::DpctlExecute(uint64_t dpId, const std::string textCmd)
{
    ...
    if (!swtch)
    {
        ...
        NS_LOG_DEBUG("Scheduling command for an unregistered switch.");
...
void
OFSwitch13Controller::DpctlSendAndPrint(struct vconn* vconn, struct ofl_msg_header* msg)
{
...
OFSwitch13Controller::StartApplication()
{
    ...
    m_serverSocket->SetAcceptCallback(MakeCallback(&OFSwitch13Controller::SocketRequest, this),
                                      MakeCallback(&OFSwitch13Controller::SocketAccept, this));
    m_serverSocket->SetCloseCallbacks(MakeCallback(&OFSwitch13Controller::SocketPeerClose, this),
...
OFSwitch13Controller::StopApplication()
{
    ...
    for (const auto& it : m_dpIdSwMap)
    {
        Ptr<RemoteSwitch> swtch = it.second;
        swtch->m_handler = nullptr;
    }
                                   ...
                                   uint32_t xid)
{
    ...
    char* msgStr = ofl_msg_to_string(msg, nullptr);
    NS_LOG_DEBUG("TX to switch " << swtch->GetIpv4() << " [dp " << swtch->GetDpId()
                                 << "]: " << msgStr);
    free(msgStr);
...
void
OFSwitch13Controller::SendEchoRequest(Ptr<const RemoteSwitch> swtch, size_t payloadSize)
{
    NS_LOG_FUNCTION(this << swtch);
...
ofl_err
OFSwitch13Controller::HandleEchoRequest(struct ofl_msg_echo* msg,
...
ofl_err
OFSwitch13Controller::HandleEchoReply(struct ofl_msg_echo* msg,
                                      Ptr<const RemoteSwitch> swtch,
                                      uint32_t xid)
{
    NS_LOG_FUNCTION(this << swtch << xid);
    ...
    if (it == m_echoMap.end())
    {
        NS_LOG_WARN("Echo response for unknown echo request.");
    }
    else
    {
        it->second.m_waiting = false;
        it->second.m_recv = Simulator::Now();
        NS_LOG_INFO("Echo reply from " << it->second.m_swtch->GetIpv4() << " with RTT "
                                       << it->second.GetRtt().As(Time::MS));
        m_echoMap.erase(it);
    }
...
ofl_err
OFSwitch13Controller::HandleBarrierReply(struct ofl_msg_header* msg,
                                         Ptr<const RemoteSwitch> swtch,
                                         uint32_t xid)
{
    NS_LOG_FUNCTION(this << swtch << xid);
    ...
    {
        NS_LOG_WARN("Barrier response for unknown barrier request.");
    }
    else
    {
...
ofl_err
OFSwitch13Controller::HandleHello(struct ofl_msg_header* msg,
                                  Ptr<const RemoteSwitch> swtch,
                                  uint32_t xid)
{
    NS_LOG_FUNCTION(this << swtch << xid);

    // We got the hello message from the switch. Let's proceed with the
    // handshake and request the switch features.
    ofl_msg_free(msg, nullptr);
...
ofl_err
OFSwitch13Controller::HandleFeaturesReply(struct ofl_msg_features_reply* msg,
...
ofl_err
OFSwitch13Controller::HandlePacketIn(struct ofl_msg_packet_in* msg,
                                     Ptr<const RemoteSwitch> swtch,
                                     uint32_t xid)
{
    NS_LOG_FUNCTION(this << swtch << xid);
...
ofl_err
OFSwitch13Controller::HandleError(struct ofl_msg_error* msg,
                                  Ptr<const RemoteSwitch> swtch,
                                  uint32_t xid)
{
    NS_LOG_FUNCTION(this << swtch << xid);

    char* msgStr = ofl_msg_to_string((struct ofl_msg_header*)msg, nullptr);
...
ofl_err
OFSwitch13Controller::HandleGetConfigReply(struct ofl_msg_get_config_reply* msg,
                                           Ptr<const RemoteSwitch> swtch,
                                           uint32_t xid)
{
    NS_LOG_FUNCTION(this << swtch << xid);
...
ofl_err
OFSwitch13Controller::HandleFlowRemoved(struct ofl_msg_flow_removed* msg,
                                        Ptr<const RemoteSwitch> swtch,
                                        uint32_t xid)
{
    NS_LOG_FUNCTION(this << swtch << xid);
...
ofl_err
OFSwitch13Controller::HandlePortStatus(struct ofl_msg_port_status* msg,
                                       Ptr<const RemoteSwitch> swtch,
                                       uint32_t xid)
{
    NS_LOG_FUNCTION(this << swtch << xid);
...
ofl_err
OFSwitch13Controller::HandleAsyncReply(struct ofl_msg_async_config* msg,
                                       Ptr<const RemoteSwitch> swtch,
                                       uint32_t xid)
{
    NS_LOG_FUNCTION(this << swtch << xid);
...
ofl_err
OFSwitch13Controller::HandleMultipartReply(struct ofl_msg_multipart_reply_header* msg,
                                           Ptr<const RemoteSwitch> swtch,
                                           uint32_t xid)
{
    NS_LOG_FUNCTION(this << swtch << xid);
...
ofl_err
OFSwitch13Controller::HandleRoleReply(struct ofl_msg_role_request* msg,
                                      Ptr<const RemoteSwitch> swtch,
                                      uint32_t xid)
{
    NS_LOG_FUNCTION(this << swtch << xid);
...
ofl_err
OFSwitch13Controller::HandleQueueGetConfigReply(struct ofl_msg_queue_get_config_reply* msg,
                                                Ptr<const RemoteSwitch> swtch,
                                                uint32_t xid)
{
    NS_LOG_FUNCTION(this << swtch << xid);
...
/********** Private methods **********/
ofl_err
OFSwitch13Controller::HandleSwitchMsg(struct ofl_msg_header* msg,
                                      Ptr<RemoteSwitch> swtch,
                                      uint32_t xid)
{
    NS_LOG_FUNCTION(this << swtch << xid);
    ...
    case OFPT_PACKET_IN:
        return HandlePacketIn((struct ofl_msg_packet_in*)msg, swtch, xid);

    case OFPT_ECHO_REQUEST:
        return HandleEchoRequest((struct ofl_msg_echo*)msg, swtch, xid);

    case OFPT_ECHO_REPLY:
        return HandleEchoReply((struct ofl_msg_echo*)msg, swtch, xid);

    case OFPT_ERROR:
        return HandleError((struct ofl_msg_error*)msg, swtch, xid);

    case OFPT_FEATURES_REPLY:
        return HandleFeaturesReply((struct ofl_msg_features_reply*)msg, swtch, xid);

    case OFPT_GET_CONFIG_REPLY:
        return HandleGetConfigReply((struct ofl_msg_get_config_reply*)msg, swtch, xid);

    case OFPT_FLOW_REMOVED:
        return HandleFlowRemoved((struct ofl_msg_flow_removed*)msg, swtch, xid);

    case OFPT_PORT_STATUS:
        return HandlePortStatus((struct ofl_msg_port_status*)msg, swtch, xid);

    case OFPT_GET_ASYNC_REPLY:
        return HandleAsyncReply((struct ofl_msg_async_config*)msg, swtch, xid);

    case OFPT_MULTIPART_REPLY:
        return HandleMultipartReply((struct ofl_msg_multipart_reply_header*)msg, swtch, xid);

    case OFPT_ROLE_REPLY:
        return HandleRoleReply((struct ofl_msg_role_request*)msg, swtch, xid);

    case OFPT_QUEUE_GET_CONFIG_REPLY:
        return HandleQueueGetConfigReply((struct ofl_msg_queue_get_config_reply*)msg, swtch, xid);
...
OFSwitch13Controller::ReceiveFromSwitch(Ptr<Packet> packet, Address from)
{
    ...
    error = ofl_msg_unpack((uint8_t*)buffer->data, buffer->size, &msg, &xid, nullptr);

    if (!error)
    {
        Ptr<RemoteSwitch> swtch = GetRemoteSwitch(from);
        char* msgStr = ofl_msg_to_string(msg, nullptr);
        Ipv4Address swIpv4 = swtch->GetIpv4();
        uint64_t swDpId = swtch->GetDpId();
        NS_LOG_DEBUG("RX from switch " << swIpv4 << " [dp " << swDpId << "]: " << msgStr);
        free(msgStr);
...
OFSwitch13Controller::SocketAccept(Ptr<Socket> socket, const Address& from)
{
    ...
    // sending/receiving OpenFlow messages to/from sockets in an independent
...
Ipv4Address
OFSwitch13Controller::RemoteSwitch::GetIpv4() const
{
    return InetSocketAddress::ConvertFrom(m_address).GetIpv4();
}

uint16_t
OFSwitch13Controller::RemoteSwitch::GetPort() const
{
    return InetSocketAddress::ConvertFrom(m_address).GetPort();
}

uint64_t
OFSwitch13Controller::RemoteSwitch::GetDpId() const
{
    return m_dpId;
}
...
Time
OFSwitch13Controller::EchoInfo::GetRtt() const
{
    if (m_waiting)
    {
        return Time(-1);
