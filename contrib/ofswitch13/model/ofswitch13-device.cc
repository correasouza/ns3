...
OFSwitch13Device::AddSwitchPort(Ptr<NetDevice> portDevice)
{
    ...
    Ptr<OFSwitch13Port> ofPort = CreateObject<OFSwitch13Port>(m_datapath, portDevice, this);
...
void
OFSwitch13Device::ReceiveFromSwitchPort(Ptr<Packet> packet, uint32_t portNo, uint64_t tunnelId)
{
    ...
    // Consume tokens, fire trace source and schedule the packet to the
    // pipeline.
...
OFSwitch13Device::NotifyConstructionCompleted()
{
    ...
    // Set the attribute values again so it can now update the datapath structs.
...
OFSwitch13Device::DatapathTimeout(struct datapath* dp)
{
    ...
    uint64_t addTokens = m_cpuCapacity.GetBitRate() * elapsedTime.GetSeconds();
...
void
OFSwitch13Device::SendToPipeline(Ptr<Packet> packet, uint32_t portNo, uint64_t tunnelId)
{
    NS_LOG_FUNCTION(this << packet << portNo << tunnelId);
    ...
    struct packet* pkt = packet_create(m_datapath, portNo, buffer, tunnelId, false);
...
OFSwitch13Device::ReceiveFromController(Ptr<Packet> packet, Address from)
{
    ...
    if (error)
    {
        // The BOFUSS library only unpacks messages that has the same
...
OFSwitch13Device::SocketCtrlSucceeded(Ptr<Socket> socket)
{
    ...
    // process of sending/receiving OpenFlow messages to/from sockets in an
    // independent way. So, each socket has its own socket handler to this end.
...
OFSwitch13Device::NotifyMeterEntryCreated(struct meter_entry* entry)
{
    ...
    // Update meter entry last_fill field with the time of last datapath
    // timeout, and force a new bucket refill based on this elapsed time.
