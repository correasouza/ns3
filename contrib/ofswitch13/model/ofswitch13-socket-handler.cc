      ...
      m_txQueue()
{
    ...
    socket->SetSendCallback(MakeCallback(&OFSwitch13SocketHandler::Send, this));
    socket->SetRecvCallback(MakeCallback(&OFSwitch13SocketHandler::Recv, this));
}
...
OFSwitch13SocketHandler::Recv(Ptr<Socket> socket)
{
    ...
    {
        // If we don't have pending bytes from an incomplete OpenFlow messages
        // it means that this is the start of a new message.
        if (!m_pendingBytes)
        {
