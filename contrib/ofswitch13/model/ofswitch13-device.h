...
class OFSwitch13Device : public Object
{
  private:
    /**
     * \ingroup ofswitch13
     * Inner class to save information of a remote active controller connected
  ...
  public:
    ...
    /**
     * Add a 'port' to the switch device. This method adds a new switch port to
     * a OFSwitch13Device, so that the new switch port NetDevice becomes part of
     ...
     */
    static void SendPacketToController(struct pipeline* pl,
                                       struct packet* pkt,
                                       uint8_t tableId,
                                       uint8_t reason);
     ...
     */
    static void DpActionsOutputPort(struct packet* pkt,
                                    uint32_t outPort,
                                    uint32_t outQueue,
                                    uint16_t maxLength,
                                    uint64_t cookie);
  ...
  private:
     ...
     * \return 0 if everything's ok, otherwise an error number.
     */
    int SendPacketInMessage(struct packet* pkt,
                            uint8_t tableId,
                            uint8_t reason,
                            uint16_t maxLength,
                            uint64_t cookie = 0);
     ...
     * used to update the initial number of tokens for this meter. Doing this,
     * we avoid the problem of discarding the initial packets before the next
     * datapath timeout.
     * \param entry The new created meter entry.
     */
